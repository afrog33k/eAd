...
function LoadMosaic(id) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(data.positions,
                                function () {
                                    AddPosition(this.Name, this.X, this.Y, this.Width, this.Height, this.PositionID, this.Media,this.ContentURL);
                                });
                                ResetDefaults();
                                if (data.background != "") {
...
function getAndSetNewPosition(name) {
    if (name == null) {
        ...
        AddPosition(name, 0, 0, 100, 100, -1, null,null);
        ResetDefaults();
    }
}
...
function AddPosition(name, x, y, nwidth, nheight, id, media,background) {
    ...
    if (background != null) {
        var url = "url(" + background + ") top left no-repeat";
        $('#' + jsName).css("background", url);
  ...
  //  if(name!="Location Info" && name!="Car Info" && name!="Battery Info"&&"Profile Info")
    $('#' + jsName).resizable({
...
//    $("#gallery").height(gHeight);
    ...
    {
        LoadDropDownList("../Mosaic/GetMosaicListProfile", "MosaicID");
        if(id>0) {
            $('#MosaicID').val(id);
        }
        else {
