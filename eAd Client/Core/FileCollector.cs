...
namespace ClientApp.Core
{
    class FileCollector
    {
        ...
        /// </summary>
        public void GetFile(long off=0, long size=DefaultChunkSize)
        {
            ...
            else
            {
                ...
                System.Diagnostics.Debug.WriteLine(String.Format("Getting the file : {0} chunk : {1} TID: {2}", _currentFileList.Path, _currentFileList.ChunkOffset.ToString(),Thread.CurrentThread.ManagedThreadId));

                // Request the file
                xmdsFile.GetFileAsync(Settings.Default.ServerKey, hardwareKey.Key, _currentFileList.Path, _currentFileList.FileType, off, size, Settings.Default.Version);
                Console.WriteLine("Downloading("+off+","+size+")");
        ...
        private static RequiredFileModel _currentFileList;
        private ServiceClient xmdsFile;
        private  const int DefaultChunkSize=1024000;
